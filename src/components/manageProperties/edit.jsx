...
const EditProperty = () => {
    ...
    useEffect(() => {
        ...
        const fetchPropertyDetails = async () => {
            try {
                console.log(`Fetching property details from: ${config.getPropertyById}/${id}`);
                const response = await axios.get(`${config.getPropertyById}/${id}`);

                console.log("API Response:", response.data);

                if (response.status === 200 && response.data.success) {
                    setFormData(response.data.data); // ✅ Populate form
                    setSelectedCity(response.data.data.city || ""); // ✅ Set selected city
                } else {
                    toast.error("Failed to fetch property details!");
                }
            } catch (error) {
                console.error("API Fetch Error:", error);
                toast.error("Error fetching property: " + (error.response?.data?.message || error.message));
            }
        };
    ...
    const validationSchema = Yup.object().shape({
        ...
        societyName: Yup.string().required("Socity Name is required"),
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // ✅ Validate form using Yup before sending data
            ...
            console.log("Sending updated data to:", `${config.updateProperty}/${id}`);

            const response = await axios.put(`${config.updateProperty}/${id}`, formDataToSend, {
            ...
            if (response.status === 200 && response.data.message === "Property updated successfully") {
                toast.success("Property updated successfully!");
                navigate("/manageProperty"); // ✅ Redirect on success
            } else {
                                        ...
                                        <div>
                                            ...
                                            <div className="row mt-4">
                                                ...
                                                <div className="col-lg-4">
                                                    ...
                                                    <input type="text" name="societyName" placeholder="Enter Socity Name" className="form-control" value={formData.societyName || ""} onChange={onInputChange} required />
                                                </div>
                                            ...
                                            {formData.images && formData.images.length > 0 && (
                                                <div className="d-flex flex-wrap gap-2 mb-3">
                                                    {formData.images.map((img, index) => (
                                                        <img
                                                            key={index}
                                                            src={typeof img === "string" ? img : URL.createObjectURL(img)} // Handle both existing & new images
                                                            alt={`Image ${index + 1}`}
